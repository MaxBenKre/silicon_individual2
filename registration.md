...
- **0.2 Plain-language summary ★** — We use an LLM to predict a distribution of possible responses for each persona and survey item. We sample directly from those normalized, uncalibrated probabilities, then use grid-aware within-bin draws to reflect the rounded response patterns common in human slider answers. This preserves model uncertainty without applying the temperature calibration used by our primary entry.
...
- **0.4 Pipeline diagram** — Personas → verbatim condition text and full questionnaire → QSTN battery prompt → verbalized distribution → validation and fallback resolution → seeded direct bin selection → grid-heaped within-bin sampling → recoding and composite construction → output CSV.
...
- **F.2 Aggregation rule** — Qwen reports a probability for each possible response bin. After validation, nonzero vectors are normalized and sampled directly without temperature scaling or any other softmax transformation. We draw one response bin per persona and item using seed 20260804. For 0–100 slider items, we redraw an integer within the selected bin using a separate generator seeded with 20260805 and relative weights of 1 for ordinary integers, 1.7 for multiples of 5 that are not multiples of 10, and 3.5 for multiples of 10. The weights are normalized separately within each bin. Donation and newsletter responses remain unchanged from the initial discrete draws. There is one model completion per persona and no averaging across completions.

## G · Validation & post-processing

- **G.1 Human validation** — N/A
- **G.2 Post-processing** — Parse JSON; attempt JSON repair if direct parsing fails; require expected item and bin keys and finite non-negative probabilities; normalize nonzero vectors that do not sum to one. Missing or invalid item distributions fall back in order to the condition mean, global mean, then uniform distribution. The normalized model probabilities are sampled directly, followed by grid-heaped within-bin sampling for 0–100 sliders. No persona is excluded; the output contains 18,000 rows across 17 conditions. Raw and parsed records retain validation metadata.
- **G.3 Calibration corrections** — N/A. This entry applies no CCAM-derived calibration and no temperature scaling; model probability vectors are used at T = 1 after normalization. Grid-heaping changes only the exact value within a selected slider bin and preserves every selected bin.
...
- **J.1 Design-space search †** — The same probability-prediction approach was evaluated on 20 Climate Change in the American Mind questions across ten waves from 2020–2024 as part of the broader design process. This secondary entry intentionally retains the uncalibrated T = 1 probabilities as an ablation of the primary calibrated approach; it adds only the grid-heaped within-bin draw. No benchmark outcome data informed this choice.

## K · Reproducibility & frozen artifacts

- **K.1 Code & materials** — https://github.com/dess-mannheim/silicon_sampling_benchmark; relevant files include the probability setup, prompt, configuration, persona manifest, launcher, and grid-heaping generator. This deposit contains predictions/team_6_T1_secondary-1_v1.csv. The source repository records both post-processing seeds. The code DOI can be found in metadata.json after the Zenodo upload.
...
- **K.3 Computational resources** — Final probability production run 6830 used local two-GPU vLLM inference and no API calls. Elapsed wall-clock time was 33 h 58 m 52 s from run launch (2026-08-11 08:49:14) to writing the raw, parsed, and T = 1 prediction files (2026-08-12 18:48:06); the QSTN battery itself completed after 33 h 54 m 31 s. Grid-heaped within-bin sampling was a deterministic post-processing step and required no second model run. A representative rendered QSTN battery prompt tokenizes to 2,957 Qwen tokens, implying approximately 53,226,000 input tokens for 18,000 personas; this is an approximation because condition-text lengths vary. At the 15,000-token generation limit, the output-token upper bound is 270,000,000.
